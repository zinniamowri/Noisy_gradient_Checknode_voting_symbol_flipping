...
numEta = length(eta_list);

FE         = zeros(numEta,1);
genFrame   = zeros(numEta,1);
iters_cnr  = zeros(numEta,1);
BE_Coded   = zeros(numEta,1);
BE_unCoded = zeros(numEta,1);

FER_vs_eta  = zeros(numEta,1);
BER_vs_eta   = zeros(numEta,1);
AvgIt_vs_eta = zeros(numEta,1);
...
for e_idx = 1:numEta
    ...
    while (FE(e_idx) < targetFE && genFrame(e_idx) < max_gen)

        genFrame(e_idx) = genFrame(e_idx) + 1;
        ...
        BE_Coded(e_idx) = BE_Coded(e_idx) + bit_error;

        iters_cnr(e_idx) = iters_cnr(e_idx) + l;
    ...
    FER_vs_eta(e_idx)   = FE(e_idx) / genFrame(e_idx);
    BER_vs_eta(e_idx)   = BE_Coded(e_idx) / (genFrame(e_idx) * K * p);
    AvgIt_vs_eta(e_idx) = iters_cnr(e_idx) / genFrame(e_idx);

    fprintf('eta=%.1f, Eb/No=%.1f dB: FER=%.3e, BER=%.3e, AvgIt=%.2f\n', ...
        eta, EbNo_dB_fixed, FER_vs_eta(e_idx), BER_vs_eta(e_idx), AvgIt_vs_eta(e_idx));
end

figure;
semilogy(eta_list, BER_vs_eta, 'o-', 'LineWidth', 1.2);
grid on;
xlabel('eta');
ylabel('BER');
...
figure;
semilogy(eta_list, FER_vs_eta, 's-', 'LineWidth', 1.2);
grid on;
xlabel('eta');
ylabel('FER');
...
figure;
plot(eta_list, AvgIt_vs_eta, 'd-', 'LineWidth', 1.2);
grid on;
xlabel('eta');
